...
// NO markup string construction. The seed it emits carries the fixed
// LOCAL_AUTHOR — every runtime's seed is the same { threads, author } shape,
// so the widget never infers which runtime it is on.

import { injectionFragment, COMMENTS_WIDGET_SRC } from '../../review-ux/inject';
import { LOCAL_AUTHOR } from './author';
import type { Thread } from '../../review-ux/types';

/**
 * Inject the comment widget into an HTML string by splicing the shared
 * injection fragment (seed JSON of this doc's threads + the fixed local
 * author + widget script tag) before </body>.
 */
