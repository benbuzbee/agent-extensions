...
#
# src/comments/ is a synced vendored copy of the skill's DOM-free shared comment
# sources (regenerate with `npm run sync:comments` from upstream). It lives inside
# the app root, so it rides this rsync like any other src/ file — no special case.
